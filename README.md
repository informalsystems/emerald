...
First, generate the testnet configuration directory with your private keys:
...
Or using the JSON format from `malachitebft-eth-app init`:

```bash
# Using JSON format (from init command output)
./scripts/generate_testnet_config.sh \
...
> - **JSON format**: The format generated by `malachitebft-eth-app init` command with base64-encoded key
>
> You can extract keys from existing nodes by reading the JSON files in `nodes/*/config/priv_validator_key.json`
