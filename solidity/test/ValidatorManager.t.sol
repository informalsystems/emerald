...
contract ValidatorManagerTest is Test {
    ...
    uint256 internal constant ALICE_PRIVATE_KEY =
    ...
    function keysEqual(
        ValidatorManager.Secp256k1Key memory a,
        ValidatorManager.Secp256k1Key memory b
    ) internal pure returns (bool) {
        return a.x == b.x && a.y == b.y;
    }

    function assertKeyEq(
        ValidatorManager.Secp256k1Key memory actual,
        ValidatorManager.Secp256k1Key memory expected
    ) internal pure {
    ...
    event ValidatorRegistered(
        ...
        ValidatorManager.Secp256k1Key validatorKey,
        uint64 power
    );
    ...
    function setUp() public {
        ...
        ValidatorManager.Secp256k1Key memory aliceKeyMem = validatorManager
            ._secp256k1KeyFromBytes(ALICE_UNCOMPRESSED);
        aliceKey = aliceKeyMem;
        aliceValidatorAddress = validatorManager._validatorAddress(aliceKeyMem);

        ValidatorManager.Secp256k1Key memory bobKeyMem = validatorManager
            ._secp256k1KeyFromBytes(BOB_COMPRESSED);
        bobKey = bobKeyMem;
        bobValidatorAddress = validatorManager._validatorAddress(bobKeyMem);

        ValidatorManager.Secp256k1Key memory coffeeKeyMem = validatorManager
            ._secp256k1KeyFromBytes(COFFEE_COMPRESSED);
        coffeeKey = coffeeKeyMem;
        coffeeValidatorAddress = validatorManager._validatorAddress(
            coffeeKeyMem
        );
    }
    ...
    function testOwnerCanRegisterValidator() public {
        ...
        ValidatorManager.ValidatorInfo memory info = validatorManager
            .getValidator(aliceValidatorAddress);
        ...
        ValidatorManager.ValidatorInfo[] memory validators = validatorManager
            .getValidators();
    ...
    function testOwnerCanRegisterSetOfValidators() public {
        ...
        ValidatorManager.ValidatorRegistration[]
            memory registrations = new ValidatorManager.ValidatorRegistration[](
                2
            );
        registrations[0] = ValidatorManager.ValidatorRegistration({
            publicKey: ALICE_UNCOMPRESSED,
            power: INITIAL_POWER
        });
        registrations[1] = ValidatorManager.ValidatorRegistration({
            publicKey: BOB_COMPRESSED,
            power: SECOND_POWER
        });
        ...
        assertEq(
            validatorManager.getTotalPower(),
            INITIAL_POWER + SECOND_POWER
        );

        ValidatorManager.ValidatorInfo memory aliceInfo = validatorManager
            .getValidator(aliceValidatorAddress);
        assertKeyEq(aliceInfo.validatorKey, aliceKey);
        assertEq(aliceInfo.power, INITIAL_POWER);
        assertTrue(validatorManager.isValidator(aliceValidatorAddress));

        ValidatorManager.ValidatorInfo memory bobInfo = validatorManager
            .getValidator(bobValidatorAddress);
    ...
    function testOwnerCanAddAndRemoveValidators() public {
        ...
        ValidatorManager.ValidatorRegistration[]
            memory addRegistrations = new ValidatorManager.ValidatorRegistration[](
                1
            );
        addRegistrations[0] = ValidatorManager.ValidatorRegistration({
            publicKey: COFFEE_COMPRESSED,
            power: THIRD_POWER
        });
        ...
        ValidatorManager.ValidatorInfo memory info = validatorManager
            .getValidator(coffeeValidatorAddress);
