...
use super::bls12381::{Bls12381, MinSig};
use super::Hashable;
...
impl PublicKey {
    ...
    pub fn verify(&self, data: &[u8], signature: &Signature) -> bool {
        let Ok(sig) = min_sig::Signature::from_bytes(&signature.bytes) else {
            return false;
        };
        let Ok(pk) = min_sig::PublicKey::from_bytes(&self.bytes) else {
            return false;
        };

        sig.verify(true, data, DST_BLS_SIG_IN_G1_WITH_POP, &[], &pk, true)
...
impl SigningScheme for Bls12381<MinSig> {
...
mod tests {
    ...
    fn sign_and_verify_roundtrip() {
        ...
        let blst_key =
            min_sig::SecretKey::key_gen(&ikm, &[]).expect("key_gen should succeed with 32 bytes");
